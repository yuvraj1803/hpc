    ...
    for(int i = 0; i < A->r; i++) {
        for(int k = 0; k < A->c; k++) {
            for(int j = 0; j + 3< B->c; j+=4) {
                ...
                out->data[i * out->c + j + 1] += A->data[i * A->c + k] * B->data[k * B->c + j + 1];
                out->data[i * out->c + j + 2] += A->data[i * A->c + k] * B->data[k * B->c + j + 2];
                out->data[i * out->c + j + 3] += A->data[i * A->c + k] * B->data[k * B->c + j + 3];
            }
        }
    }
}
